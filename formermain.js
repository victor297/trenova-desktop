...
// Clean up temp files when the app closes
...
console.log("fileName",fileName,"downloadPath",downloadPath )
...
ipcMain.handle('getFile', async (event, filePath) => {
  try {
    ...
    openFiles.add(tempPath) // Track the temp file
    return `safe-file://${tempPath}`
  } catch (error) {
...
ipcMain.handle('deleteFile', async (event, filePath) => {
  try {
    // Close any open handles to temp files
    cleanupTempFiles()
    ...
    // Use a try-catch block for the actual deletion
    try {
      fs.unlinkSync(filePath)
    } catch (error) {
      // If the first attempt fails, try with a small delay
...
ipcMain.handle('deleteAllFiles', async () => {
  try {
    // Clean up any open temp files first
    cleanupTempFiles()
